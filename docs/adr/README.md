# Architecture Decision Records

This directory captures the current ferrovia decisions in descending importance.

## Order

- `0001` product and compatibility target
- `0002` core document model and parser strategy
- `0003` verification and regression oracle
- `0004` public surfaces and packaging
- `0005` config and plugin model
- `0006` delivery and commit discipline
- `0007` wave ordering and commit strategy
- `0008` tree rewrite mutation rules
- `0009` minimal semantic collections
- `0010` dependency leverage and OSS adoption
- `0011` minimal css stylesheet model
- `0012` semantic direct-port spike
- `0013` direct-port rewrite reset

New ADRs should continue the numeric sequence. If a later decision supersedes an older one, add a new ADR and link back to the superseded document instead of rewriting history.
